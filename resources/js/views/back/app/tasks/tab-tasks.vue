<template>
    <div class="flex flex-col">
        <div class="mb-6">
            <div class="mt-1 flex rounded-md shadow-sm">
                <div class="relative flex-grow focus-within:z-10">
                    <input class="form-input rounded-r-none" placeholder="Enter a new task.." v-model="newSubTask.content" @keyup.enter="addTask()"/>
                </div>
        ...
        <div class="flex flex-col items-center" v-if="subtasks.length === 0">
            <v-empty-view message="There are no tasks to show here"/>
        </div>

        <div class="flex flex-col mb-4" v-if="incompletedTasks.length > 0">
            <div class="pb-2 border-b border-gray-200 mb-1">
                <h4 class="font-medium text-lg">Todo ({{ incompletedTasks.length }})</h4>
            </div>
        ...
        <div class="flex flex-col mb-4" v-if="completedTasks.length > 0">
            <div class="pb-2 border-b border-gray-200 mb-1">
                <h4 class="font-medium text-lg">Completed ({{ completedTasks.length }})</h4>
            </div>
